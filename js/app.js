...
function Dashboard({
 ...
 /* v18: percenkénti frissítés, hogy az időfüggő emlékeztetők maguktól megjelenjenek */
 const [nowTick, setNowTick] = useState(0);
 useEffect(() => {
  const iv = setInterval(() => setNowTick(t => t + 1), 60000);
  return () => clearInterval(iv);
 }, []);

 /* ═══ v18 (6.6): ÉTKEZÉS UTÁNI MÉRÉSI EMLÉKEZTETŐ — kapcsolható, alapból BE ═══ */
 const mealRemind = (() => {
  if (!onNoteSave || (settings && settings.featMealRemind === false)) return null; /* követőnél nem jelenik meg */
  const mins = Math.min(240, Math.max(30, parseInt(settings && settings.mealRemindMin) || 100));
  ...
  try {
   if (localStorage.getItem('hbc-mealremind-done') === String(lastMeal.id)) return null;
  } catch (x) {}
  return {
   id: lastMeal.id,
   elapsed: Math.round(elapsed),
   fatProt: !!lastMeal.fatProt
  };
 })();

 /* ═══ v18 (6.7): HETI ÖSSZEFOGLALÓ A KÖVETŐNEK — kapcsolható, alapból BE ═══ */
 const weeklyOn = (!settings || settings.featWeekly !== false) &&
  window.HBC_SYNC && window.HBC_SYNC.cfg && window.HBC_SYNC.cfg.mode === 'follower' &&
  (() => {
   try {
    const last = parseInt(localStorage.getItem('hbc-weekly-last') || '0');
 ...
 return h('div', {
  ...
  /* ═══ v18 (6.6): étkezés utáni mérési emlékeztető ═══ */
  mealRemind && h('div', {
    className: 'p-3 rounded-2xl bg-teal-50 border-2 border-teal-200 text-teal-800 text-sm font-bold flex items-center justify-between gap-2 flex-wrap'
   },
   h('span', null, '⏰ ' + window.t('Az utolsó étkezés óta') + ' ' + mealRemind.elapsed + ' ' + window.t('perc telt el — ideje ellenőrző mérést végezni!') +
    (mealRemind.fatProt ? ' 🥓 ' + window.t('(Zsíros/fehérjedús étel volt: 2–3 óra múlva is érdemes mérni.)') : '')),
   h('button', {
    type: 'button',
    onClick: () => {
     try {
      localStorage.setItem('hbc-mealremind-done', String(mealRemind.id));
     } catch (x) {}
     setNowTick(t => t + 1);
    },
    className: 'px-3 py-1 rounded-xl bg-teal-600 text-white text-xs font-bold'
   }, '✓ ' + window.t('Rendben'))),

  /* ═══ v18 (6.7): heti összefoglaló a követőnek ═══ */
  weeklyOn && bgAll7.length > 0 && h('div', {
   ...
   h('div', {
    ...
    }, '📬 ' + window.t('Heti összefoglaló (utolsó 7 nap)')),
    h('button', {
     type: 'button',
     onClick: () => {
      try {
       localStorage.setItem('hbc-weekly-last', String(Date.now()));
      } catch (x) {}
      setNowTick(t => t + 1);
     },
     className: 'px-3 py-1 rounded-xl bg-white/20 text-white text-xs font-bold'
    }, '✓ ' + window.t('Rendben'))),
   h('div', {
    ...
    h('div', null, h('p', {
     className: 'text-2xl font-black'
    }, bgAll7.filter(e => parseFloat(e.bloodGlucose) < _dLow).length), h('p', {
     className: 'text-xs opacity-80'
    }, window.t('hipó')))
   )),
...
${email?'':'<scr'+'ipt>setTimeout(function(){window.print();},400);</scr'+'ipt>'}
 ...
 if (email) return {
  ...
  data: {
   ...
   chAvg: (sumCH / dayCount).toFixed(1) + ' g',
...
function Statistics({
 ...
 /* ═══ v18 (6.2): AGP JELLEGŰ PERCENTILIS GRAFIKON ═══
    A nemzetközi szabvány szerinti nézet: óránkénti medián + a mérések középső
    50%-át (25–75. percentilis) és 90%-át (5–95. percentilis) mutató sávok.
    Ujjbegyes mérések ÉS betöltött CGM-adatok együtt. Kapcsolható a Beállításokban. */
 const agpOn = !settings || settings.featAGP !== false;
 const agpHourly = useMemo(() => {
  if (!agpOn) return null;
  const buckets = Array.from({
   length: 24
  }, () => []);
  bgE.forEach(e => buckets[new Date(e.timestamp).getHours()].push(parseFloat(e.bloodGlucose)));
  const cgmRows = window.HBC_CGM ? window.HBC_CGM.getRange(f, t) : [];
  cgmRows.forEach(r => {
   const v = parseFloat(r.v);
   if (!isNaN(v) && v > 0) buckets[new Date(r.ts).getHours()].push(v);
  });
  const pct = (sorted, p) => {
 ...
 useEffect(() => {
  if (!agpOn || !agpR.current || !agpHourly || !agpHourly.nAll) return;
  agpC.current && agpC.current.destroy();
  const _dark = document.documentElement.hasAttribute('data-hbc-dark');
  Chart.defaults.color = _dark ? '#c9c5e8' : '#666';
  const D = window.bgU.disp;
  const rowV = k => agpHourly.rows.map(x => x && x[k] != null ? D(x[k]) : null);
  agpC.current = new Chart(agpR.current.getContext('2d'), {
   type: 'line',
   data: {
    labels: Array.from({
     length: 24
    ...
    datasets: [{
     ...
     },
     {
      ...
      fill: {
       target: '-1',
       above: 'rgba(124,58,237,.10)',
       below: 'rgba(124,58,237,.10)'
      }
     },
     {
      label: '75%',
     ...
     },
     {
      ...
      fill: {
       target: '-1',
       above: 'rgba(124,58,237,.25)',
       below: 'rgba(124,58,237,.25)'
      }
     },
     {
      label: window.t('Medián'),
      data: rowV('p50'),
      borderColor: '#7c3aed',
      borderWidth: 2.5,
      pointRadius: 0,
   ...
   options: {
    ...
    plugins: {
     legend: {
      position: 'top',
      labels: {
       filter: it => ['Medián', 'Median'].includes(it.text) || it.text.startsWith('Max') || it.text.startsWith('Min')
      }
     }
    }
   }
 ...
 return h('div', {
  ...
  /* ═══ v18 (6.2): AGP percentilis nézet — kapcsolható a Beállításokban ═══ */
  agpOn && agpHourly && agpHourly.nAll > 0 && card([
   h('h3', {
    className: 'font-black text-gray-800 mb-3'
   }, '📈 ' + window.t('AGP nézet (percentilis sávok)')),
   h('div', {
    className: 'chart-container'
   }, h('canvas', {
    ref: agpR
   ...
   h('p', {
    className: 'text-xs text-gray-400 mt-2'
   }, window.t('A vastag vonal az óránkénti középérték (medián). A sötétebb sáv a mérések középső 50%-a (25–75. percentilis), a halványabb a középső 90%-a (5–95. percentilis). Ujjbegyes és CGM-adatok együtt') + ` (${agpHourly.nAll} ${window.t('mérés')}).`)
  ]),
    ...
    shown.map(f => h('div', {
     ...
     h('div', {
      ...
      h('div', null,
       ...
       }, `${fmtCH(f.carbs)}g CH${f.unit?' / '+f.unit:''}`)
      ),
      h('div', {
        className: 'flex gap-1'
       },
  ...
  h('form', {
   ...
   h('div', null,
    ...
    h('div', {
     ...
     /* v18 (6.4): Bluetooth mérő — az utolsó mérés átvétele (Android/Chrome; iPhone-on nem támogatott) */
     (settings && settings.featBT !== false) && window.HBC_BT && h('button', {
      type: 'button',
      title: window.t('Mérés átvétele Bluetooth vércukormérőből'),
   ...
   (form.type === 'Étkezés' || form.type === 'Kontroll') && h('div', null,
    ...
    }, `TELJES CH: ${fmtCH(totalCH)}g`),
    /* ═══ v18 (6.3): ZSÍROS/FEHÉRJEDÚS ÉTEL JELÖLÉS — tájékoztató, dózistanács nélkül ═══ */
    (settings && settings.featFatProt !== false) && h('div', {
      className: 'mt-2'
     },
     h('label', {
     ...
     form.fatProt && h('p', {
      className: 'text-xs font-bold text-amber-800 mt-1 p-2 bg-amber-50 rounded-xl border border-amber-200'
     }, '⚠️ ' + window.t('A zsír és a fehérje lassítja a felszívódást: a vércukor később, elnyújtva is emelkedhet. Javasolt 2–3 óra múlva ellenőrző mérést végezni. Ez tájékoztatás, nem dózistanács — kérdéseddel fordulj kezelőorvosodhoz!')))
   ),
  ...
  h('div', {
   ...
   h('form', {
    ...
    h('div', null,
     ...
     totalCH > 0 && h('p', {
      className: 'text-xs font-black text-indigo-600 mt-1'
     }, window.t('TELJES CH') + `: ${fmtCH(totalCH)}g`),
     /* v18 (6.3): zsíros/fehérjedús jelölés utólagos szerkesztésnél is */
     showFoodEd && h('label', {
       className: 'flex items-center gap-2 text-sm font-bold text-amber-800 mt-2'
      },
      h('input', {
       type: 'checkbox',
       checked: !!form.fatProt,
...
/* v18: motivációs üzenetek alapkészlete — a Beállításokban szerkeszthető */
const MOTIV_MSG_DEFAULTS = [
 '✅ Szuper! Céltartományban vagy! 👏',
 '✨ Minden bejegyzéssel önmagadért teszel — csodásan csinálod!',
 '🌟 A tested hálás minden odafigyelésért!',
 '🧘 Egyensúly és harmónia — jó úton jársz!',
 '💪 A rendszeresség a legjobb gyógyítód!',
 '🌈 Apró lépések, nagy változások — büszke lehetsz magadra!'
];
/* v18: a Sorozat-kártya feliratai — pozitív/ezoterikus alapszövegek, a Beállításokban szerkeszthetők */
const STREAK_DEFAULTS = {
 title: '🔥 Sorozat',
 suffix: 'napja áramlik veled az egyensúly energiája',
...
const DEFAULT_SETTINGS = {
 ...
 /* v18: motivációs üzenetek — szerkeszthető lista + megjelenési idő (mp, 1–10) */
 motivMsgs: MOTIV_MSG_DEFAULTS.slice(),
 motivDur: 3,
 /* v18: Sorozat-kártya feliratai — szerkeszthetők */
 streakTitle: STREAK_DEFAULTS.title,
 streakSuffix: STREAK_DEFAULTS.suffix,
 streakMsg: STREAK_DEFAULTS.msg,
 /* v18: új szolgáltatások kapcsolói — alapból mind BEkapcsolva */
 featCHTable: true,   /* beépített magyar szénhidráttáblázat */
 featAGP: true,       /* AGP percentilis grafikon */
 featFatProt: true,   /* zsíros/fehérjedús étel figyelmeztetés */
 featBT: true,        /* Bluetooth vércukormérő kapcsolat */
 featPredLow: true,   /* előrejelzett alacsony érték riasztás */
 featMealRemind: true,/* étkezés utáni mérési emlékeztető */
...
function Settings({
 ...
 return h('div', {
  ...
  /* ═══ v18: MOTIVÁCIÓS ÜZENETEK ÉS SOROZAT-FELIRATOK ═══ */
  card([
   h('h2', {
    className: 'font-black text-indigo-700 mb-1'
   }, '💬 ' + t('Motivációs üzenetek')),
   h('p', {
    className: 'text-sm text-gray-500 mb-3'
   }, t('Ezek az üzenetek jelennek meg bejegyzés mentése után, ha az érték céltartományban van. Soronként egy üzenet — szabadon átírhatod, törölheted, bővítheted.')),
   h('textarea', {
    value: (Array.isArray(s.motivMsgs) ? s.motivMsgs : MOTIV_MSG_DEFAULTS).join('\n'),
    onChange: e => setS({
     ...s,
     motivMsgs: e.target.value.split('\n')
    }),
    rows: 6,
    className: 'w-full border-2 border-indigo-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-indigo-400 bg-white'
   }),
   h('div', {
    ...
    h('p', {
     className: 'text-xs text-gray-400'
    }, t('Üres sorok nem számítanak. Ha minden sort törölsz, az alapüzenetek élnek.')),
    h('button', {
     type: 'button',
     onClick: () => setS({
      ...s,
   ...
   h('div', {
    ...
    h('label', {
      className: 'text-sm font-bold text-indigo-700 block mb-1'
     }, '⏱️ ' + t('Megjelenítés hossza') + ': ' +
     (Math.min(10, Math.max(1, parseFloat(s.motivDur) || 3))) + ' ' + t('mp')),
    h('input', {
     type: 'range',
     min: '1',
     max: '10',
     step: '1',
     value: Math.min(10, Math.max(1, parseFloat(s.motivDur) || 3)),
     onChange: e => setS({
      ...s,
      motivDur: parseInt(e.target.value) || 3
     }),
    ...
    h('div', {
      className: 'flex justify-between text-[10px] text-indigo-400 font-bold'
     },
     h('span', null, '1 ' + t('mp')), h('span', null, '5 ' + t('mp')), h('span', null, '10 ' + t('mp')))),
   h('h2', {
    className: 'font-black text-indigo-700 mb-1'
   }, '🔥 ' + t('Sorozat feliratai')),
   h('p', {
    className: 'text-sm text-gray-500 mb-3'
   }, t('Az Áttekintés Sorozat-kártyáján megjelenő szövegek — fogalmazd a saját ízlésedre!')),
   h('div', {
    ...
    h('div', null,
     ...
     h('input', {
      type: 'text',
      value: s.streakTitle != null ? s.streakTitle : STREAK_DEFAULTS.title,
      onChange: e => setS({
       ...s,
       streakTitle: e.target.value
      }),
      className: fi
     })),
    h('div', null,
     h('label', {
      className: 'text-sm font-bold text-indigo-700 block mb-1'
     }, t('Szám utáni felirat')),
     h('input', {
      type: 'text',
      value: s.streakSuffix != null ? s.streakSuffix : STREAK_DEFAULTS.suffix,
      onChange: e => setS({
       ...s,
       streakSuffix: e.target.value
      }),
      className: fi
     })),
    h('div', null,
     h('label', {
      className: 'text-sm font-bold text-indigo-700 block mb-1'
     }, t('Buzdító üzenet (3+ napnál)')),
     h('input', {
      type: 'text',
      value: s.streakMsg != null ? s.streakMsg : STREAK_DEFAULTS.msg,
      onChange: e => setS({
       ...s,
       streakMsg: e.target.value
      }),
      className: fi
     }))),
   h('button', {
   ...
   h('div', {
    ...
    [{
     ...
     o.k === 'featMealRemind' && s.featMealRemind !== false && h('div', {
      ...
      }, t('Jelzés az étkezés után') + ':'),
      h('input', {
       type: 'number',
       min: 30,
       max: 240,
       step: 5,
       value: s.mealRemindMin != null ? s.mealRemindMin : 100,
       onChange: e => setS({
        ...s,
        mealRemindMin: parseInt(e.target.value) || 100
       }),
       className: 'w-20 border-2 border-indigo-200 rounded-xl px-2 py-1 text-sm focus:outline-none'
      }),
      h('span', {
       className: 'text-xs text-gray-500'
      }, t('perc') + ' (30–240, ' + t('javasolt') + ': 90–120)'))
    ))
   ),
   /* nagy betűs mód — megjelenítési opció, alapból KI, hogy a megszokott nézet ne változzon */
   h('div', {
     className: 'mt-4 p-3 rounded-xl border-2 border-indigo-200 bg-indigo-50/50'
    },
    h('label', {
      className: 'flex items-center gap-2 text-sm font-bold text-gray-800'
     },
     h('input', {
      type: 'checkbox',
      checked: !!s.featBigFont,
      onChange: e => setS({
       ...s,
       featBigFont: e.target.checked
      }),
      className: 'w-5 h-5 accent-indigo-600'
     }),
     '🔠 ' + t('Nagy betűs mód')),
...
function App() {
 ...
 /* v18: a felugró üzenet hossza a Beállításokban állítható (1–10 mp) */
 const showToast = msg => {
  setToast(msg);
  const sec = Math.min(10, Math.max(1, parseFloat(settings && settings.motivDur) || 3));
  setTimeout(() => setToast(null), sec * 1000);
 };
 ...
 useEffect(() => {
  ...
  /* v18 (6.7): nagy betűs mód */
  document.documentElement.classList.toggle('hbc-bigfont', !!settings.featBigFont);
 ...
 const addEntry = e => {
  ...
  if (e.bloodGlucose && parseFloat(e.bloodGlucose) < low && isToday) {
   ...
   if (e.bloodGlucose && parseFloat(e.bloodGlucose) >= low && parseFloat(e.bloodGlucose) <= high) {
    /* v18: véletlenszerű üzenet a Beállításokban szerkeszthető listából */
    const _msgs = (Array.isArray(settings.motivMsgs) ? settings.motivMsgs : MOTIV_MSG_DEFAULTS).map(x => String(x).trim()).filter(Boolean);
    const _pool = _msgs.length ? _msgs : MOTIV_MSG_DEFAULTS;
    showToast(_pool[Math.floor(Math.random() * _pool.length)]);
   } else {
