...
window.HBC_SYNC = (function() {
 ...
 let cfg = {
  ...
  fileName: '',
  minutes: 5,
  accountEmail: '' /* v14: automatikus bejelentkezés — a megadott Google-fiókkal, fiókválasztó nélkül */
 };
 ...
 function loadCfg() {
  try {
   Object.assign(cfg, JSON.parse(localStorage.getItem('hbc-drive') || '{}'));
  } catch (e) {}
 ...
 function saveCfg() {
  localStorage.setItem('hbc-drive', JSON.stringify(cfg));
  emit('status');
 }
 ...
 function loadScript(src) {
  return new Promise((res, rej) => {
   ...
   s.src = src;
   s.onload = res;
   s.onerror = () => rej(new Error('Nem tölthető be: ' + src));
 ...
 function requestToken(promptVal) {
  return new Promise((res, rej) => {
   /* v14: automatikus bejelentkezés — ha a Tulajdonos megadta a fiók e-mail címét,
      a Google a fiókválasztó LISTA HELYETT azonnal ezt a fiókot használja.
      Rosszullét esetén egy idegen segítőnek sem kell 10+ fiókból választania.
      (Az első engedélyezés eszközönként egyszer továbbra is kézzel történik.) */
   const initCfg = {
   ...
   if (cfg.accountEmail) initCfg.login_hint = cfg.accountEmail.trim();
   tokenClient = google.accounts.oauth2.initTokenClient(initCfg);
 ...
 function clearToken() {
  accessToken = null;
  tokenExp = 0;
  localStorage.removeItem('hbc-drive-token');
  localStorage.removeItem('hbc-drive-exp');
  localStorage.removeItem('hbc-drive-scope');
  sessionStorage.removeItem('hbc-drive-token');
  sessionStorage.removeItem('hbc-drive-exp');
 }
 ...
 function api(path, opts) {
  ...
  return fetch('https://www.googleapis.com/' + path, opts).then(r => {
   if (r.status === 401) {
    clearToken();
   } /* lejárt/visszavont token → következő hívás újat kér */
   if (!r.ok) {
    const e = new Error('Drive API hiba: ' + r.status);
    e.status = r.status;
    throw e;
   }
   return r;
 ...
 function merge(remote, local) {
  ...
  arr(remote.deleted).concat(arr(local.deleted)).forEach(t => {
   ...
   const k = (t.k || 'e') + ':' + t.id;
   if (!tomb[k] || (t.ts || 0) > tomb[k].ts) tomb[k] = {
    id: t.id,
    ts: t.ts || 0,
    k: t.k || 'e'
   };
  ...
  const cg = {};
  arr(remote.cgm).concat(arr(local.cgm)).forEach(r => {
   if (r && r.ts) cg[r.ts] = r;
  });
 ...
 function push(payloadObj) {
  ...
  pendingPush = setTimeout(() => {
   doPush(payloadObj).catch(() => {
    /* offline: később pótlódik */ });
  }, 4000);
 }
 ...
 function pull(manual) {
  ...
  const run = ensureToken(!!manual, !manual && pull._quiet)
   ...
   .then(r => r.json())
   .then(j => {
    /* v14: PRIVÁT bejegyzések — a Tulajdonos által 🔒 priváttá tett bejegyzés
       a Követő számára TELJESEN rejtett (lista, statisztika, riasztás). */
    if (j && Array.isArray(j.entries)) j.entries = j.entries.filter(e => !(e && e.private));
    cfg.lastSync = new Date().toISOString();
    saveCfg();
    emit('data', j);
    checkAlerts(j);
 ...
 function checkAlerts(data) {
  try {
   ...
   /* ═══ v14: OPCIONÁLIS értesítés MINDEN új bejegyzésről (követő telefonján) ═══
      Az azonosítók időbélyeg-alapúak (Date.now), így az eddig látott legnagyobb
      id-nél nagyobb id = új bejegyzés. Az első letöltéskor csak megjegyezzük az
      állapotot (nincs visszamenőleges értesítés-özön), a privát bejegyzések ide
      el sem jutnak (a pull már kiszűrte őket). */
   try {
    if (al.entryOn) {
     const ids = (data.entries || []).map(e => parseInt(e && e.id)).filter(n => !isNaN(n));
     const curMax = ids.length ? Math.max.apply(null, ids) : 0;
     const prev = parseInt(localStorage.getItem('hbc-entry-notif-max') || '0');
     if (prev > 0 && curMax > prev) {
      const news = (data.entries || []).filter(e => parseInt(e && e.id) > prev);
      const newest = news.slice().sort((x, y) => new Date(y.timestamp) - new Date(x.timestamp))[0];
      const en = window.HBC_I18N.getLang() === 'en';
      const body = news.length > 1 ?
       (en ? news.length + ' new entries' : news.length + ' új bejegyzés') :
       (window.t((newest && newest.type) || '') + ' – ' + new Date(newest.timestamp).toLocaleString());
      new Notification('HBC Napló – ' + window.t('Új bejegyzés érkezett'), {
       body: '📝 ' + body,
       icon: 'icons/icon-192.png',
       tag: 'hbc-entry'
      });
     }
     if (curMax > prev) localStorage.setItem('hbc-entry-notif-max', String(curMax));
    }
   } catch (e) {}
   ...
   const low = a.lowBG != null ? a.lowBG : 3.9,
    high = a.highBG != null ? a.highBG : 10.0;
 ...
 window.addEventListener('online', () => {
  cfg.mode === 'follower' ? pull() : ownerAutoSync();
 });
 ...
 return {
  ...
  merge,
  checkAlerts,
  /* v14: riasztáslogika kívülről is hívható (teszteléshez) */
  startPolling,
  stopPolling,
  setPayloadProvider(fn) {
   payloadProvider = fn;
  },
  setMode(m) {
  ...
  isConfigured() {
   return !!(cfg.clientId && (cfg.folderId || cfg.fileId));
  }
 };
})();